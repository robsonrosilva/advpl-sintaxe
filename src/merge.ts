...
export class MergeAdvpl {
  ...
  public merge(branchDestino: string): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (
        objeto.branchesControladas.includes(this.branchOrigem.toUpperCase())
      ) {
      ...
      objeto
        ...
        .then(() => {
          ...
          this.run([
            ...
            .then(() => {
              // efetua os merges
              this.mergeGit(this.branchTeste)
                .then(() => {
                  if (
                    [this.branchHomol, this.branchProdu].includes(branchDestino)
                  ) {
                    this.mergeGit(this.branchHomol)
                      .then(() => {
                        if (this.branchProdu === branchDestino) {
                          this.mergeGit(this.branchProdu)
                            .then((tag: string) => {
  ...
  public atualiza(): Promise<any> {
    // se não estiver com a branch origem definida define ela
    this.branchOrigem = this.branchOrigem
      ? this.branchOrigem
      : this.repository.headLabel;
    return new Promise((resolve, reject) => {
      // vai para a branche de release
      this.repository
        .checkout(this.branchHomol)
        .then(() => {
          ...
          this.repository
            ...
            .then(() => {
              ...
              this.repository
                .checkout(this.branchOrigem)
                .then(() => {
                  // efetua o merge
                  this.run(["merge", "--no-ff", this.branchHomol])
                    .then(() => {
  ...
  public limpaBranches(): Promise<any> {
    return new Promise((resolve, reject) => {
      // Atualiza todas as branches remotas
      this.run(["fetch", "-v", "origin"])
        .then(() => {
          // baixa todas as tags
          this.run(["pull", "--tags"])
            .then(() => {
              ...
              this.run(["remote", "prune", "origin"])
                .then(() => {
                  ...
                  this.run(["branch", "--merged", "master"])
                    .then((ret: IExecutionResult<string>) => {
                      ...
                      branches.forEach((branche: string) => {
                        ...
                        if (
                          branche.substring(0, 1) !== "*" &&
                          !this.branchesControladas.includes(
                            branche.substring(2).toUpperCase()
                          ) &&
                          branche !== ""
                        ) {
                    ...
                    .catch((erro) => {
                      return reject(
                        localize("merge.mergedError") + "\n" + erro.stderr
                      );
                    });
                })
                .catch((erro) => {
                  return reject(
                    localize("merge.remotePruneError") + "\n" + erro.stderr
                  );
                });
            })
            .catch((erro) => {
  ...
  private pushAll(): Promise<any> {
    ...
    promises.push(
      this.run(["push", "--set-upstream", "origin", this.repository.headLabel])
    );
        ...
        .then(() => {
          ...
          this.run(["pull"])
            .then(() => {
              ...
              const branch: string =
                destino === this.branchProdu
                  ? this.branchHomol
                  : this.branchOrigem;

              // efetua o merge
              this.run(["merge", "--no-ff", branch])
                .then(() => {
